"""
Facebook Token model for OAuth flow.
"""
import datetime
...
class FacebookToken(Base):
    ...
    expires_in = Column(String(50))  # seconds from Facebook

    # Refresh token (if available)
    refresh_token = Column(Text)

    # Token metadata
    granted_scopes = Column(String(500))
    """
    Comma-separated list of granted scopes:
    "ads_management,ads_read,pages_manage_ads,pages_read_engagement"
    """
    ...
    # Relationships
    user = relationship("User", backref="facebook_tokens")
    dealership = relationship("Dealership", backref="facebook_tokens")

    def __repr__(self) -> str:
        return f"<FacebookToken user={self.user_id} active={self.is_active}>"
